...
const getTexPrompt = () => `
    ...
    2. **Intelligent Structuring**: Structure the content using appropriate LaTeX commands. Identify headings (\\section{}, \\subsection{}), lists (\\begin{itemize}, \\begin{enumerate}), and mathematical equations. For inline math, use $...$. For displayed equations, especially multi-line derivations, ALWAYS use a suitable environment like \\begin{align*}...\\end{align*} to ensure proper alignment and vertical spacing. Do not crowd equations together; give them room to breathe.
    ...
    4. **Problem Solving & Derivations**: If the notes contain questions, exercises, or an assignment, you must solve them completely. For any derivation or problem, provide a clear, step-by-step explanation for each part of the process. The explanation should precede or be interleaved with the mathematical steps.
...
const getPdfPrompt = () => `
    ...
    2. **Content Enhancement & Elaboration**: This is the most important step. You must enhance the notes by adding detail, providing in-depth explanations, and clarifying concepts to make them comprehensive. For any problem or derivation, provide a clear, step-by-step explanation for each part of the process. The explanation should be in 'paragraph' blocks, interleaved with the mathematical steps.
    ...
    6. **Equations**: This is critical for readability. Any mathematical formula or equation that should be displayed on its own line must be placed in a content block with \`type: 'equation'\`. For multi-step derivations, use a separate 'equation' block for EACH step/line to ensure they are rendered line-by-line with proper spacing. The 'text' field should contain the equation as a plain string (e.g., 'E = mc^2').
    7. **Structure and Format**: Structure the entire enhanced content into a logical sequence of elements (headings, paragraphs, lists, equations, and diagrams) and return it as a single JSON object conforming to the provided schema.
        ...
        content: {
            ...
            items: {
                ...
                properties: {
                    type: { 
                        type: Type.STRING, 
                        description: "Type of content block. Can be 'heading1', 'heading2', 'paragraph', 'bullet_list', 'diagram', or 'equation'." 
                    },
                    text: { 
                        type: Type.STRING, 
                        description: "The text content for a heading, paragraph, equation, or a diagram's textual description." 
                    },
                    items: {
                        type: Type.ARRAY,
